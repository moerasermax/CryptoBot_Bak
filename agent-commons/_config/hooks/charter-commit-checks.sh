...
check_h3() {
    # 接受大寫 — 對齊 framework individual-learning-loop §2.4 範例 `F1_pm-fake-claim`（F 大寫）。
    # 原 lowercase-only regex 與 framework 範例 + 既存 PM reflection（如 `F1_F3_a003-...`）衝突。
    local regex='^[0-9]{4}-[0-9]{2}-[0-9]{2}_[A-Za-z0-9_-]+\.md$'
    for f in $STAGED_FILES; do
        case "$f" in
            "$CMR"/roles/*/reflections/*.md)
